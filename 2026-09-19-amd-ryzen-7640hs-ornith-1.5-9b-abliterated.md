...
| 测试 | 结果 | 平均耗时 | 平均输出 |
|---|---:|---:|---:|
| GSM8K 普通模式，前 10 题（第 1 次） | 4/10 | 14.4 秒 | 150.2 tokens |
| GSM8K 普通模式，前 10 题（第 2 次，配置完全相同） | 5/10 | 12.3 秒 | 127.2 tokens |
| GSM8K 思考模式，前 3 题 | 3/3 | 15.7 秒 | 169.3 tokens |
| BFCL v4 simple_python，前 20 题 | 16/20 | 5.6 秒 | 55.3 tokens |
| IFEval 前 10 题 Strict（默认设置 `temp=0`） | 3/10，9/18 | 93.9 秒 | 1071.6 tokens |
| IFEval 前 10 题 Strict（**卡片推荐采样**） | **4/10，11/18** | 70.9 秒 | 802.6 tokens |
| IFEval 前 10 题 Loose（卡片推荐采样） | 4/10，11/18 | 同上 | 同上 |
| IFEval，卡片推荐采样 + **开启思考** | **无法完成** | — | — |
...
| **Ornith-1.5-9B-OBLITERATED（llama.cpp/AMD）** | **4–5/10** | **3/3** | **16/20** | **4/10** | **11/18** |

**一句话结论：工具调用和思考数学是并列最好的，普通数学中游，严格指令遵循
（IFEval）是全部被测模型里最差的**——即使换用官方推荐采样，也只从 3/10 升到 4/10。
